...
function check_ok(element){
    var here = $(element);
    //here.closest('li').find('.minus').click();
    //here.closest('li').find('.plus').click();
}

$('body').on('click','.quantity-button', function(){
    var here = $(this);
    var quantity = here.closest('li').find('.quantity_field').val();
    var limit = here.closest('li').find('.quantity_field').data('limit');
    //notify('minus','info');
    if(here.val()=='minus'){
        quantity = quantity-1;
    ...
    if(quantity >= 1){
        //here.closest('li').find('.quantity_field').val(quantity);

        var rowid = here.closest('li').find('.quantity_field').data('rowid');
        var lim_t = here.closest('li').find('.limit');
        var list1 = here.closest('ul').find('.sub_total');
        list1.html('...');
        $.ajax({
            ...
            success: function(data) {
                ...
                others_count();
                if(res[1] !== 'not_limit'){
                    notify(quantity_exceeds,'warn');
                    lim_t.html('!!').fadeIn();
                    here.closest('li').find('.plus').hide();
                    here.closest('li').find('.quantity_field').data('limit','yes');
                    here.closest('li').find('.quantity_field').val(res[1]);
                } else {
                    lim_t.html('').fadeOut();
                    here.closest('li').find('.plus').show();
                    here.closest('li').find('.quantity_field').data('limit','no');
                    here.closest('li').find('.quantity_field').val(res[2]);
                }
            },
            error: function(e) {
                console.log(e)
...
$('.quantity-button').unbind('click').click(function() {
    //alert("bob");
...
function cart_submission(){
    ...
    //window.location="http://localhost/rawsqueezed/application/views/front/home/home1/send.php";
}
...
function update_calc_cart(){
    ...
    $.ajax({
        ...
        success: function(data) {
            var res = data.split('-');
            total.html('<i class="fa fa-inr" aria-hidden="true">' + res[0] + '</i>').fadeIn();
            ship.html('<i class="fa fa-inr" aria-hidden="true">' + res[1] + '</i>').fadeIn();
            tax.html(res[2]).fadeIn();
            grand.html('<i class="fa fa-inr" aria-hidden="true">'+res[3] + '</i>').fadeIn();
            //other_action();
        },
        error: function(e) {
            console.log(e)
    ...
    $.ajax({
        ...
        success: function(result){
            if(result == 'nope'){
                ...
                notify('Unavailable','warning','bottom','right');
            } else {
                // notify(result,'warning','bottom','right');
