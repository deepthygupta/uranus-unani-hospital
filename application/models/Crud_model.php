...
class Crud_model extends CI_Model
{
    ...
    function file_up_product($name, $type, $id, $cat, $multi = '', $no_thumb = '', $ext = '.jpg')
    {
        if ($multi == '') {
            $ib = 1;
            foreach ($_FILES[$name]['name'] as $i => $row) {
                $ib = $this->file_exist_ret($type, $id, $cat, $ib);
                move_uploaded_file($_FILES[$name]['tmp_name'][$i], 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $cat . '_' . $ib . $ext);
                if ($no_thumb == '') {
                    $this->crud_model->img_thumb($type, $id . '_' . $cat . '_' . $ib, $ext);
                }
            }
        } elseif ($multi == 'multi') {
            //echo $name;
        ...
        } elseif ($multi == 'multi_edit') {
            //echo $name;
            $ib = $cat;
            $old_cat = $this->db->get_where('product', array('product_id' => $id))->row()->category;
            foreach ($_FILES[$name]['name'] as $i => $row) {
                //$ib = $this->file_exist_ret_edit($type, $id,$old_cat, $ib);
//                move_uploaded_file($_FILES[$name]['tmp_name'][$i], 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $ib . $ext);
                rename('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $old_cat . $ext, 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $ib . $ext);
                if ($no_thumb == '') {
                    $this->crud_model->img_thumb($type, $id . '_' . $ib, $ext);
                }
            }
...
//    function file_exist_ret_edit($type, $id, $ib,$old_cat, $ext = '.jpg')
//    {
//        if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $old_cat . $ext)) {
//            $ib = $ib + 1;
//            $ib = $this->file_exist_ret($type, $id, $ib);
//            return $ib;
//        } else {
...
//        }
//    }
    function file_view1($type, $id, $cat_id, $width = '100', $height = '100', $thumb = 'no', $src = 'no', $multi = '', $multi_num = '', $ext = '.jpg')
    {
        if ($multi == '') {
            if (file_exists('uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $cat_id . $ext)) {
                if ($thumb == 'no') {
                    $srcl = base_url() . 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $cat_id . $ext;
                } elseif ($thumb == 'thumb') {
                    $srcl = base_url() . 'uploads/' . $type . '_image/' . $type . '_' . $id . '_' . $cat_id . $ext;
                }
    ...
    function product_link1($product_id)
    {

        $name = url_title($this->crud_model->get_type_name_by_id('product', $product_id, 'title'));
        return base_url() . 'index.php/home/product_view1/' . $product_id . '/' . $name;
    ...
    function update_counter()
    {
        ...
        $time = time();
        //echo $data['ip_address'] . "haaai";
        $this->db->insert('visitors', $data);
        $val = $this->db->get_where('visitors', array('ip_address' => $data['ip_address'],
            'time_stamp >= ' => $today_start, 'time_stamp <= ' => $today_end))->num_rows();
        if ($val == 0) {
            $expip = $this->db->get_where('visitors', array('ip_address' => $data['ip_address']))->num_rows();
            if ($expip != 0) {
...
>
    ...
    */
    ...
    function get_total_from_sale($sale_code)
    {
        $carted = $this->crud_model->get_product_details_sale($sale_code);
        $total = 0;
        foreach ($carted as $items) {
            $total += $items['subtotal'];
        }
    ...
    function email_invoice_new()
    {
    }

    function email_verification($data = '', $user_id = '')
    {
        //echo 'user_id:' . $user_id . ',code:' . $data;
        $code = $this->db->get_where('user', array('user_id' => $user_id))->row()->email_verification;
        $verify_time = $this->db->get_where('user', array('user_id' => $user_id))->row()->email_verify_time;
        // echo 'veri'. $verify_time;
        $time = time();
        if ($code == 'verified') // if already verified exit
            return 'already';
        if ($data == 'generate') { // generate token
            $code = md5(mt_rand() * 45634567890345676) . md5(mt_rand() * 8557856345335);
            $this->db->where('user_id', $user_id);
            $this->db->update('user', array('email_verification' => $code,
                'email_verify_time' => strtotime('tomorrow')));
            return $code;
        } else {
            if (($verify_time - $time) < 0) {
                $code1 = md5(mt_rand() * 45634567890345676) . md5(mt_rand() * 8557856345335);
                $this->db->where('user_id', $user_id);
                $this->db->update('user', array('email_verification' => $code1));
                return 'expired';
            }
            if ($data == $code) {  // email cod equal to value
                $this->db->where('user_id', $user_id);
                $this->db->update('user', array('email_verification' => 'verified'));
                $this->referral_after_confirm($user_id);
                return 'success';
            } else {   // email not equal
                return 'failed';
            }
    ...
    /**
     * @param string $do code
     * @param string $user_id referring user
     * @param string $applied_user_id beneficial user id when signup
     * @return string
     */
    function referral($do = '', $user_id = '', $applied_user_id = '')
    {
        //echo 'code: ' . $do . ',user_id:' . $user_id . ',aid:' . $applied_user_id;
        $verified = $this->db->get_where('user', array('user_id' => $user_id))->row()->email_verification;
        $ap_verified = $this->db->get_where('user', array('user_id' => $applied_user_id))->row()->email_verification;
        if ($user_id == $applied_user_id)
            return 'referring self';
        if ($ap_verified == 'verified')
            return 'user already created';
        if (empty($do) || $verified != 'verified')
            return 'empty or not verified';
        if ($do == 'generate') {
            $code = substr(md5(mt_rand()), 0, 10);
            $this->db->where('user_id', $user_id);
            $this->db->update('user', array('referral_token' => $code));
            return 'generated';
        } else {
            //echo 'applied user1 '.$applied_user_id;
            $code = $this->db->get_where('user', array('user_id' => $user_id))->row()->referral_token;
            if (empty($applied_user_id))
                return 'empty user'; //when appied user id is empty
            if ($do == $code) {
                $count = (int)$this->db->get_where('user', array('user_id' => $user_id))->row()->referral_times;
                //echo $count;
                if ($count == '')
                    $count = 0;
                if ($count < 10) {
                    //echo 'applied user '.$applied_user_id;
                    $count = $count + 1;
                    $this->db->where('user_id', $user_id);
                    $this->db->update('user', array('referral_times' => $count));
                    //echo 'applied user '.$applied_user_id;
                    $this->db->where('user_id', $applied_user_id);
                    $this->db->update('user', array('referral_used' => $user_id));
                    return 'success';
                } else {
    ...
    function referral_after_confirm($user_id)
    {
        $ref_limit=$this->db->get_where('refer', array('refer_id' => 1))->row()->refer_limit;
        $ref_amount=$this->db->get_where('refer', array('refer_id' => 1))->row()->amount;
        $max_amount=$ref_amount*$ref_limit;
        $verified = $this->db->get_where('user', array('user_id' => $user_id))->row()->email_verification;
        if ($verified != 'verified')
            return 'not verified';
        $ref_user = $this->db->get_where('user', array('user_id' => $user_id))->row();
        $ref_user_id = $ref_user->referral_used;
        $ref_amount_count = $this->db->get_where('user', array('user_id' => $ref_user_id))->row()->referral_amount_count;
//        echo 'refer amt'. $ref_amount_count .'ref'.$ref_user_id;
        if ($ref_amount_count == '')
            $ref_amount_count = 0;
//        echo 'refer amt n'. $ref_amount_count;
        if ($ref_amount_count < $max_amount) {
            $ref_amount_count = $ref_amount_count + $ref_amount;
            $this->db->where('user_id', $ref_user_id);
            $this->db->update('user', array('referral_amount_count' => $ref_amount_count));
            return 'discount_added';
        } else return 'amount out of limit';

    }

    function get_email_template($data = '', $oid, $name, $odt, $phone, $deltime_date, $delv_note, $email, $add, $total, $shipping, $city, $state, $zip, $payment, $order_status)
    {
...
<body>
<div style="text-align: center;"><div style="border:black solid 1px;height:auto;width: 500px;">
    <div>
<!--        <center> <img src=--><?php //base_url().\'uploads/logo/logo-dark.png\'?><!--</center>-->

    </div>
    <table class="">
        <tbody>
        <tr>
            <td style="padding: 5px 10px;" width="150">Order ID: </td>
            <td style="padding: 5px 10px;">' . $oid . '</td>
        </tr>
        <tr>
            <td style="padding: 5px 10px;" width="150">Order Date & Time: </td>
            <td style="padding: 5px 10px;">' . $odt . '</td>
        </tr>
        <tr>
            <td style="padding: 5px 10px;" width="150">Delivery Date & Time: </td>
            <td style="padding: 5px 10px;">' . $deltime_date . '</td>
        </tr>
        <tr>
            <td style="padding: 5px 10px;" width="150">Delivery Note: </td>
            <td style="padding: 5px 10px;">' . $delv_note . '</td>
        </tr>
...
<br/>
<table style="border:black solid 1px;height:auto;width: 500px;">
        <tbody>
        
        <tr>
            <td style="padding: 5px 10px;" width="150">Customer Name: </td>
        ...
        <tr>
            <td style="padding: 5px 10px;" width="150">Shipping Address: </td>
            <td style="padding: 5px 10px;">' . $add . ' </td>
        </tr>
        <tr>
        ...
        <tr>
            <td style="padding: 5px 10px;" width="150">State: </td>
            <td style="padding: 5px 10px;">' . $state . ' </td>
        </tr>
        <tr>
            <td style="padding: 5px 10px;" width="150">Zip: </td>
            <td style="padding: 5px 10px;">' . $zip . ' </td>
        </tr>
        <tr>
            <td style="padding: 5px 10px;" width="150">Payment: </td>
            <td style="padding: 5px 10px;">' . $payment . ' </td>
        </tr>
               <tr>
            <td style="padding: 5px 10px;" width="150">Payment status: </td>
            <td style="padding: 5px 10px;">' . $order_status . ' </td>
        </tr>
        </tbody>
...
<!--        <tr><td></td></tr>-->
    ...
    <div style="background-color: #f9faf4">
    <p style="margin-right: 1px;"> Regards,</p>
    <p style="margin-left: 10px;"> Rawsqueezed</p>
    sales@rawsqueezed.com| Call +91 7510 150 150 | www.rawsqueezed.com
    </div>
</div>
</div>
        ...
        return $dat . $data . $data_end;
    }

    function clear_all()
    {
        $this->session->unset_userdata("pin");
        $this->session->unset_userdata('sale_code');
        ...
        $this->session->unset_userdata('couponer');
    }
    ...
    {
        return '8E3352696228048C6C06F5B10BAB09AB';
    }
...
//        $shipping_address = json_decode($sale->shipping_address, true);
    ...
    function email_template($name = 'example', $link = 'test', $btn_text = 'ACTIVATE MY ACCOUNT')
    {

        ob_start();
        include APPPATH . 'third_party/email/email_verify.php';
        return ob_get_clean();
        //$this->email_model->send_email();
    }

    function get_referral_link()
    {
        $user_id = $this->session->userdata('user_id');
        if ($this->db->get_where('user', array('user_id' => $user_id))->row()->email_verification == 'verified') {
            $referral_id = $this->db->get_where('user', array('user_id' => $user_id))->row()->referral_token;
            return base_url() . 'index.php/home/login2/' . $user_id . '/' . $referral_id;
        } else return 'Please verify your email';

    }

    function sms_place_order_send($sale_code)
    {
        $sale = $this->db->get_where('sale', array('sale_code' => $sale_code))->row();
        $shipping_address = json_decode($sale->shipping_address, true);
        $grand_total = $sale->grand_total;

        $message = 'Thank you for choosing RAWSQUEEZED. Your order ' . $sale_code . ' for Rs.' . $grand_total . ' has been successfully placed. You will receive a shipping confirmation soon.';
        $this->sms_model->send_sms($shipping_address['phone'], $message);
    }

    function sms_confirm_send($sale_id)
    {
        $sale = $this->db->get_where('sale', array('sale_id' => $sale_id))->row();
        $shipping_address = json_decode($sale->shipping_address, true);
        $sale_code = $sale->sale_code;
        $message = 'We are on our way. Your order ' . $sale_code . ' has been successfully shipped out and should reach you in 2 hours. Call 7510150150 for any query.';
        $this->sms_model->send_sms($shipping_address['phone'], $message);
    }

    function order_complete_template($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status)
    {

        ob_start();
        include APPPATH . 'third_party/order/order_template.php';
        return ob_get_clean();
//        $this->email_model->send_email();
    }

    function order_confirmed_template($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status)
    {

        ob_start();
        include APPPATH . 'third_party/order/order_confirmed.php';
        return ob_get_clean();
//        $this->email_model->send_email();
    }

    function send_order_placed_email($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status)
    {
        $mass = $this->order_complete_template($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status);
        $this->email_model->send_email($email_id, 'Order #' . $sale_code . ' Placed by ' . $name . ' is ' . $order_status, $mass);
        //$this->crud_model->clear_all();
    }

    function send_order_confirmed_email($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status)
    {
        $mass = $this->order_confirmed_template($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status);
        $this->email_model->send_email($email_id, 'Order #' . $sale_code . ' Placed by ' . $name . ' is confirmed', $mass);
        //$this->crud_model->clear_all();
    }


    function password_recovery($user_id = '', $data = 'generate', $password = '')
    {
        //echo 'user_id:' . $user_id . ',code:' . $data;
        $code = $this->db->get_where('user', array('user_id' => $user_id))->row()->recover_code;
        $verify_time = $this->db->get_where('user', array('user_id' => $user_id))->row()->recovery_time_limit;
        // echo 'veri'. $verify_time;
        $time = time();
        if ($data == 'generate') { // generate token
            $code = md5(mt_rand() * 45634567890345676) . md5(mt_rand() * 8557856345335);
            $this->db->where('user_id', $user_id);
            $this->db->update('user', array('recovery_time_limit' => strtotime('tomorrow'), 'recover_code' => $code
            ));
            return $code;
        } elseif ($data == 'recovered') {
            ...
            if (($verify_time - $time) < 0) {
                $code1 = md5(mt_rand() * 45634567890345676) . md5(mt_rand() * 8557856345335);
                $this->db->where('user_id', $user_id);
                $this->db->update('user', array('recovery_time_limit' => strtotime('tomorrow'),
                    'recover_code' => $code1
                ));
                return 'expired';
            } else if ($data == $code) {  // code equal to value
                if ($password == '')
                    return "password is empty";
                $this->db->where('user_id', $user_id);
                $this->db->update('user', array('password' => sha1($password),
                    'recover_code' => 'recovered'));
    ...
    // COUPON STATUS CHECK
    function check_coupon_status($coupon_code)
    {
        return $this->db->get_where('discount', array('code' => $coupon_code))->row()->status;
    }

    function coupon_limit_count($coupon_code, $dat = false)
    {
        $count = $this->db->get_where('discount', array('code' => $coupon_code))->row()->coupon_limit;
//        $id = $this->db->get_where('discount', array('code' => $coupon_code))->row()->discount_id;
        if ($dat) {
            $count = $this->db->get_where('discount', array('discount_id' => $coupon_code))->row()->coupon_limit;
            if ($count != null && $count > 0) {
                $count--;
                $data_coupon['coupon_limit'] = $count;
            }
            $this->db->where('discount_id', $coupon_code);
            $this->db->update('discount', $data_coupon);
        }
        if ($count == null) {
            return 0;
        } else return $count;
    ...
    function fb_get_appid(){
        return '321487368309978';
    }
    function fb_get_pageid(){
        return '122420508297898';
    }
