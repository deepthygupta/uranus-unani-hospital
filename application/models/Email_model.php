...
class Email_model extends CI_Model
{
    ...
    function send_email($to, $sub, $msg)
    {
        $config = Array(
            'protocol' => 'smtp',
            'smtp_host' => 'ssl://md-in-26.webhostbox.net',
            'smtp_port' => 465,
            'smtp_user' => 'info@rawsqueezed.com',
            'smtp_pass' => 'kl07cb@1188',
            'mailtype'  => 'html',
            'charset'   => 'utf-8'
        );
        $this->load->library('email', $config);
        $this->email->set_newline("\r\n");
        $this->email->from('info@rawsqueezed.com', 'RAWSQUEEZED');
        $this->email->to($to);
        $this->email->bcc('kiranbjm@gmail.com,rawsqueezed@gmail.com');
        $this->email->subject($sub);
        $this->email->message($msg);
