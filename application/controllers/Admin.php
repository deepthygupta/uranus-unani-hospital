...
class Admin extends CI_Controller {
    ...
    function event($para1 = '', $para2 = '') {
        ...
        if ($para1 == 'do_add') {
            ...
            $page_data['event_data'] = $this->db->get_where('event', array(
                        'event_id' => $para2
                    ))->result_array();
            ...
            $page_data['all_event'] = $this->db->get('event')->result_array();
            $this->load->view('back/admin/event_list', $page_data);
        } elseif ($para1 == 'add') {
            $this->load->view('back/admin/event_add');
        } else {
      ...
      function gallery($para1 = '', $para2 = '') {
        ...
        if ($para1 == 'do_add') {
            ...
            $page_data['data'] = $this->db->get_where('gallery', array(
                        'id' => $para2
                    ))->result_array();
    ...
    function coupon($para1 = '', $para2 = '', $para3 = '') {
        ...
        if ($para1 == 'do_add') {
            ...
            $path = $_FILES['img']['name'];
            $ext = pathinfo($path, PATHINFO_EXTENSION);
            $data_coupon['image'] = 'discount_' . $id . '.' . $ext;
            $this->crud_model->file_up("img", "discount", $id, '', 'no', '.' . $ext);
            $this->db->where('coupon_id', $id);
            $this->db->update('coupon', $data_coupon);
            $this->crud_model->set_category_data(0);
            recache();
        } else if ($para1 == 'edit') {
            ...
            if ($_FILES['img']['name'] !== '') {
                $path = $_FILES['img']['name'];
                $ext = pathinfo($path, PATHINFO_EXTENSION);
                $data_banner['image'] = 'discount_' . $para2 . '.' . $ext;
                $this->crud_model->file_up("img", "discount", $para2, '', 'no', '.' . $ext);
                $this->db->where('coupon_id', $para2);
                $this->db->update('coupon', $data_banner);
            }
            $this->crud_model->set_category_data(0);
            recache();
        } elseif ($para1 == 'delete') {
    ...
    function coupon_original($para1 = '', $para2 = '', $para3 = '') {
        if (!$this->crud_model->admin_permission('coupon')) {
            redirect(base_url() . 'index.php/admin');
        }
        if ($para1 == 'do_add') {
            ...
            $data['code'] = $this->input->post('code');
            $data['till'] = $this->input->post('till');
            $data['coupon_limit'] = $this->input->post('coupon_limit');
            $data['status'] = 'ok';
            $data['added_by'] = json_encode(array('type' => 'admin', 'id' => $this->session->userdata('admin_id')));
            $data['spec'] = json_encode(array(
                'set_type' => $this->input->post('set_type'),
            ...
            $page_data['coupon_data'] = $this->db->get_where('discount', array(
                        'discount_id' => $para2
                    ))->result_array();
            $this->load->view('back/admin/coupon_original_edit', $page_data);
        } elseif ($para1 == "update") {
            $data['title'] = $this->input->post('title');
            $data['code'] = $this->input->post('code');
            $data['till'] = $this->input->post('till');
            ...
            $this->db->where('discount_id', $para2);
            ...
            $page_data['all_coupons'] = $this->db->get('discount')->result_array();
            $this->load->view('back/admin/coupon_original_list', $page_data);
        } elseif ($para1 == 'add') {
            $this->load->view('back/admin/coupon_original_add');
        } elseif ($para1 == 'publish_set') {
            $product = $para2;
            if ($para3 == 'true') {
                $data['status'] = 'ok';
            ...
            $this->db->where('discount_id', $product);
            $this->db->update('discount', $data);
        } else {
            $page_data['page_name'] = "coupon_original";
            $page_data['all_coupons'] = $this->db->get('discount')->result_array();
            $this->load->view('back/index', $page_data);
        }
    }
    ...
    function testimonial_category($para1 = '', $para2 = '') {
...
//            $page_data['testimonial_category_data'] = $this->db->get_where('testimonial_category', array(
//                'testimonial_category_id' => $para2
//            ))->result_array();
//            $this->load->view('back/admin/testimonial_category_edit', $page_data);
        } elseif ($para1 == "update") {
            $data['name'] = $this->input->post('name');
            $this->db->where('testimonial_category_id', $para2);
            ...
            $this->db->order_by('testimonial_category_id', 'desc');
            $page_data['all_categories'] = $this->db->get('testimonial_category')->result_array();
            $this->load->view('back/admin/testimonial_category_list', $page_data);
        } elseif ($para1 == 'add') {
            $this->load->view('back/admin/testimonial_category_add');
        } else {
...
//    function testimonial_edit($param1 = "")
//    {
//
//        $page_data['page_name']      = "testimonial_edit";
//        $page_data['testimonial_data'] = $this->db->get_where('testimonial', array(
//            'testimonial_id' => $param1
//        ))->result_array();
//        //$page_data['testimonial_data'] = $this->db->get('testimonial')->result_array();
//        $this->load->view('back/index', $page_data);
//
//    }
    function testimonial($para1 = '', $para2 = '') {
        if (!$this->crud_model->admin_permission('testimonial')) {
            redirect(base_url() . 'index.php/admin');
        }
        if ($para1 == 'do_add') {
            ...
            $data['author'] = $this->input->post('author');
            $data['description'] = $this->input->post('description');
            $data['des_and_company'] = $this->input->post('des_and_company');

            $this->db->insert('testimonial', $data);
            $id = $this->db->insert_id();
            $this->crud_model->file_up("img", "testimonial", $id, '', '', '.jpg');
            $this->crud_model->file_up("img", "testimonial", $id, '', '', '.jpg');
            recache();
        } else if ($para1 == 'edit') {
            $page_data['testimonial_data'] = $this->db->get_where('testimonial', array(
                        'testimonial_id' => $para2
                    ))->result_array();
            $this->load->view('back/admin/testimonial_edit', $page_data);
        } elseif ($para1 == "update") {
            $data['author'] = $this->input->post('author');
            $data['description'] = $this->input->post('description');
            $data['des_and_compnay'] = $this->input->post('des_and_compnay');
            $this->db->where('testimonial_id', $para2);
            $this->db->update('testimonial', $data);
            $this->crud_model->file_up("img", "testimonial", $para2, '', '', '.jpg');
            recache();
        } elseif ($para1 == 'delete') {
            $this->crud_model->file_dlt('testimonial', $para2, '.jpg');
            $this->db->where('testimonial_id', $para2);
            $this->db->delete('testimonial');
            recache();
        } elseif ($para1 == 'list') {
            $this->db->order_by('testimonial_id', 'desc');
            $page_data['all_testimonials'] = $this->db->get('testimonial')->result_array();
            $this->load->view('back/admin/testimonial_list', $page_data);
        } elseif ($para1 == 'add') {
            $this->load->view('back/admin/testimonial_add');
        } else {
    ...
    function product($para1 = '', $para2 = '', $para3 = '') {
        ...
        if ($para1 == 'do_add') {
            ...
            $id = $this->db->insert_id();
            $cat = $this->db->get_where('product', array('product_id' => $id))->row()->category;

            $this->benchmark->mark_time();
            $this->crud_model->file_up_product("images", "product", $id, $cat, 'multi');
//            move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/product_image/' ."product"._.$id ._ .$cat .'.jpg');
//            $this->crud_model->file_up("images", "product", $id, 'multi');
            ...
            $data['options'] = json_encode($options);
            $this->crud_model->file_up_product("images", "product", $para2, $cat, 'multi_edit');  //put this before database update
//            $this->crud_model->file_up("images", "product", $para2, 'multi');
    ...
    function sales($para1 = '', $para2 = '') {
        ...
        if ($para1 == 'delete') {
            ...
            $order_status = json_decode($this->db->get_where('sale', array(
                        'sale_id' => $para2
            ...
            foreach ($order_status as $row) {
                if (isset($row['admin'])) {
                    $new_order_status[] = array('admin' => '', 'status' => $this->input->post('order_status'));
                    if ($this->input->post('order_status') == 'accepted') {
                        $this->email_temp($para2);
                        $this->crud_model->sms_confirm_send($para2);
                    }
                } else {
                    $new_order_status[] = array('vendor' => $row['vendor'], 'status' => $row['status']);
                }
            }


            $data['order_status'] = json_encode($new_order_status);
    ...
    function newsletter($para1 = "") {
        ...
        if ($para1 == "send") {
            ...
            $this->email_model->send_email($users, $title, $text);
            $this->email_model->send_email($subscribers, $title, $text);
...
//            }
    ...
    function default_images($para1 = "", $para2 = "") {
        ...
        $page_data['default_list'] = array('product_image', 'digital_logo_image', 'category_image', 'sub_category_image', 'brand_image', 'blog_image', 'testimonial_image', 'banner_image', 'user_image', 'vendor_logo_image', 'vendor_banner_image', 'membership_image', 'slides_image');
    ...
    function email_temp($sale_id, $order_status = '') {
...
//        $this->crud_model->clear_all();
        $this->crud_model->send_order_confirmed_email($sale_code, $name, $phone, $sale, $email_id, $address, $total, $shipping_charge, $city, $state, $zip, $payment, $order_status);
